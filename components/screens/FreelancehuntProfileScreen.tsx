...
function ConnectPanel({ userId, account, onRefresh }: {
  ...
  const handleConnect = async () => {
    ...
    setError(null);
    try {
      const r = await fetch('/api/freelancehunt/connect', {
        method: 'POST',
      ...
      if (r.ok && data?.ok) {
        ...
        const detail =
          ...
          ?? (raw ? `HTTP ${r.status}: ${raw.slice(0, 140)}` : `HTTP ${r.status}`);
        setError(detail);
        setStep('error');
        haptic.error();
      }
    } catch (err) {
      setError(`Мережа: ${err instanceof Error ? err.message : String(err)}`);
      setStep('error');
      haptic.error();
    }
  };
